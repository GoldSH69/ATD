...
exports.generateImageKeywords = generateImageKeywords;
...
async function generateImageKeywords(input) {
    const settings = (0, settings_1.getSettings)();
    try {
        const system = 'You create concise image-search keywords for editorial social posts. Output only 2 to 5 plain keywords, comma separated. No hashtags.';
        ...
        const raw = await (0, llm_1.generateText)(settings.llm, system, user);
        const words = raw
            .replace(/["']/g, '')
            .split(/\n|;/)[0]
            .split(',')
...
function queryVariants(query) {
    ...
    for (let i = 0; i <= terms.length - 3; i += 1)
        add(terms.slice(i, i + 3).join(' '));
    for (let i = 0; i <= terms.length - 2; i += 1)
        add(terms.slice(i, i + 2).join(' '));
...
async function searchCommons(query) {
    ...
    for (const page of Object.values(pages)) {
        ...
        if (!imageUrl || !thumbUrl || !mime.startsWith('image/'))
            continue;
        images.push({
            url: imageUrl,
            thumbUrl,
...
async function searchImages(query) {
    const q = query.trim();
    if (!q)
        return { ok: false, images: [], message: 'Image search query is required' };
