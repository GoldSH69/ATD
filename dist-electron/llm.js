...
const asObj = (v) => (v !== null && typeof v === 'object' ? v : {});
const snippet = (text) => text.replace(/\s+/g, ' ').trim().slice(0, 200);
const stripTrailingSlashes = (u) => u.trim().replace(/\/+$/, '');
...
function validateHttpBase(base) {
    try {
...
function httpMessage(label, status, body) {
...
function parseJson(label, status, body) {
    try {
...
function modelIds(data) {
    ...
    if (!Array.isArray(list))
        return [];
    return list.map((m) => asObj(m).id).filter((id) => typeof id === 'string');
}
async function testHosted(label, url, apiKey, headers) {
    if (!apiKey.trim())
        return { ok: false, message: 'API key is required' };
    const res = await request(url, { headers }, TEST_TIMEOUT_MS);
    if (!res.ok)
        return { ok: false, message: httpMessage(label, res.status, res.body) };
...
async function testLocal(local) {
    ...
    if (!res.ok)
        return { ok: false, message: httpMessage('Local LLM', res.status, res.body) };
    ...
    if (found)
        return { ok: true, message: `Connected to ${base} — model "${model}" is available.` };
    const listed = ids.length === 0
        ? 'no models listed'
...
async function generateClaude(cfg, system, user) {
    if (!cfg.apiKey.trim())
    ...
    if (!res.ok)
        throw new Error(httpMessage('Anthropic', res.status, res.body));
...
async function chatCompletion(label, url, headers, model, system, user, 
...
sampling) {
    ...
    if (!res.ok)
        throw new Error(httpMessage(label, res.status, res.body));
...
async function generateText(llm, system, user) {
    switch (llm.provider) {
        case 'claude':
            return generateClaude(llm.claude, system, user);
        case 'openai': {
            if (!llm.openai.apiKey.trim())
                throw new Error('OpenAI API key is required');
            return chatCompletion('OpenAI', 'https://api.openai.com/v1/chat/completions', { authorization: `Bearer ${llm.openai.apiKey}` }, llm.openai.model, system, user, { max_completion_tokens: 2048 });
        }
        case 'local': {
            ...
            const invalid = validateHttpBase(base);
            if (invalid)
                throw new Error(`Local LLM: ${invalid}`);
            const apiKey = llm.local.apiKey.trim();
            return chatCompletion('Local LLM', `${base}/chat/completions`, apiKey ? { authorization: `Bearer ${apiKey}` } : {}, llm.local.model, system, user, { max_tokens: 1024, temperature: 0.8 });
        }
    }
}
