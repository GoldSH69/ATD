...
exports.generateAutopilotPost = generateAutopilotPost;
exports.generateAutopilotReply = generateAutopilotReply;
exports.decideAutopilotPlan = decideAutopilotPlan;
...
async function runGeneration(settings, userPrompt, systemPrompt) {
    try {
        const raw = await (0, llm_1.generateText)(settings.llm, systemPrompt ?? buildSystemPrompt(settings.style), userPrompt);
...
async function runAutoDraft() {
    ...
    try {
        ...
        const news = await (0, news_1.fetchTopicNews)({ query: topic, sources: settings.newsSources });
...
function languageDirective(mode, appLang, hasReference) {
    switch (mode) {
        case 'ko':
 ...
 */
function categoryVoiceHint(category) {
    if (!category)
        return null;
...
function buildPersonaPrompt(settings, kind, category) {
    ...
    if (creator) {
        lines.push(`You are an AI agent created by ${creator}. You may playfully acknowledge being an AI when it fits, but it is not your whole personality.`);
    }
    ...
    if (catHint)
        lines.push(`- Niche voice for this post: ${catHint}`);
    if (kind === 'reply') {
    ...
    if (notes)
        lines.push('', `Personality / style notes: ${notes}`);
...
async function generateAutopilotPost(input) {
    const settings = (0, settings_1.getSettings)();
    const missing = unconfiguredMessage(settings.llm);
    if (missing)
        return { ok: false, text: '', message: missing };
    ...
    if (hasReference) {
        ...
        parts.push(`Write a native "${niche}" Threads post reacting to this headline: "${input.newsTitle}"${src}.`);
        parts.push('Give ONE sharp, human angle, opinion, or joke — the kind popular accounts in this niche post. Not a summary. Not a press release.');
    }
    else {
        parts.push(`Write an ORIGINAL Threads post in the popular "${niche}" niche — the kind of post that gets replies and quotes on Threads today.`);
        parts.push('It does NOT have to be about news. Prefer shower-thought, hot take, "just tried X", tiny builder story, or a question to the timeline.');
    }
    if (input.angle)
        parts.push(`Suggested angle: ${input.angle}.`);
...
async function generateAutopilotReply(input) {
    const settings = (0, settings_1.getSettings)();
    const missing = unconfiguredMessage(settings.llm);
    if (missing)
        return { ok: false, text: '', message: missing };
    ...
    if (kind === 'mention') {
        ...
        parts.push('Write a short, natural reply to that mention — acknowledge them, answer if they asked something, and stay in character. Do not restate the whole post.');
    }
    else if (kind === 'discover') {
        parts.push('You are joining a public Threads conversation (discovery engagement). This is NOT your post — you are a friendly stranger adding value.');
        parts.push(`@${input.replyUsername} posted: "${input.rootPostText || input.replyText}".`);
        parts.push('Write ONE short, human reply: a genuine take, helpful note, or light joke. No spam, no self-promo dump, no "check my profile". Sound native to Threads.');
    }
    else {
    ...
    if (input.isCreator) {
        ...
        parts.push(`IMPORTANT: this is ${address} — the person who created you. Address them warmly as "${address}", be a little playful and deferential, and if it fits, reassure them things are running smoothly. Keep it short.`);
    }
...
function fallbackPlan(candidates, maxPosts, originalRatio) {
    const settings = (0, settings_1.getSettings)();
    const niches = settings.autopilot.categories.length > 0
 ...
 */
async function decideAutopilotPlan(input) {
    const settings = (0, settings_1.getSettings)();
    const ap = settings.autopilot;
    const maxPosts = Math.max(0, Math.floor(input.maxPosts));
    if (maxPosts === 0)
        return { items: [], reasoning: 'Daily budget reached.', usedFallback: false };
    const missing = unconfiguredMessage(settings.llm);
    if (missing)
        return { items: [], reasoning: missing, usedFallback: false };
    ...
    let raw = '';
    try {
        raw = await (0, llm_1.generateText)(settings.llm, system, user);
    }
    catch (err) {
