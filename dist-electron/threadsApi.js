...
const uid = (cfg) => cfg.userId.trim() || 'me';
...
async function publish(cfg, text, replyToId) {
    const u = uid(cfg);
    const createParams = { media_type: 'TEXT', text };
    if (replyToId)
        createParams.reply_to_id = replyToId;
    const created = await apiPost(cfg, `/${u}/threads`, createParams);
    ...
    for (let attempt = 0;; attempt++) {
        try {
            const published = await apiPost(cfg, `/${u}/threads_publish`, { creation_id: created.id });
            ...
            if (attempt >= 5 || !isTransientPublishError(err))
                throw err;
            await delay(2000);
        }
    }
...
async function publishPost(cfg, text) {
...
async function fetchReplyMessages(cfg, postId) {
    const params = { fields: 'id,text,username,timestamp,replied_to', limit: '100' };
    try {
        const data = await apiGet(cfg, `/${postId}/conversation`, params);
        return { items: data.data ?? [], topLevelOnly: false };
    }
    catch {
        const data = await apiGet(cfg, `/${postId}/replies`, params);
...
async function fetchUnansweredReplies(cfg) {
    const me = await apiGet(cfg, '/me', { fields: 'id,username' });
    ...
    for (const post of posts) {
        ...
        for (const m of items) {
            if (m.username === myUsername && typeof m.replied_to?.id === 'string')
                answeredIds.add(m.replied_to.id);
        }
        for (const m of items) {
            if (typeof m.id !== 'string' || !m.id)
                continue;
            if (!m.username || m.username === myUsername)
                continue;
            if (answeredIds.has(m.id))
                continue;
            if (!topLevelOnly && m.replied_to?.id !== post.id)
                continue;
