...
exports.testThreads = testThreads;
exports.threadsMediaExists = threadsMediaExists;
exports.publishPost = publishPost;
exports.publishReply = publishReply;
exports.fetchMyPosts = fetchMyPosts;
exports.scrapeRecentTexts = scrapeRecentTexts;
exports.searchKeywordPosts = searchKeywordPosts;
exports.fetchUnansweredMentions = fetchUnansweredMentions;
exports.fetchUnansweredReplies = fetchUnansweredReplies;
exports.fetchUnansweredEngagement = fetchUnansweredEngagement;
...
const snippet = (body) => body.replace(/\s+/g, ' ').trim().slice(0, 200) || '(empty body)';
const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
const errText = (err) => (err instanceof Error ? err.message : String(err));
async function apiFetch(cfg, method, path, params) {
...
const apiGet = (cfg, path, params) => apiFetch(cfg, 'GET', path, params);
const apiPost = (cfg, path, params) => apiFetch(cfg, 'POST', path, params);
async function testThreads(cfg) {
    try {
        const me = await apiGet(cfg, '/me', { fields: 'id,username' });
...
const isTransientPublishError = (err) => {
    const m = errText(err);
    return /not ready|not found|not finished|not available|try again|temporarily|does not exist|in progress|processing/i.test(m);
};
...
async function threadsMediaExists(cfg, mediaId) {
    if (!mediaId.trim())
        return false;
    try {
        const media = await apiGet(cfg, `/${mediaId.trim()}`, { fields: 'id' });
 ...
 */
async function publishAgainstUser(cfg, userPath, text, replyToId, imageUrl) {
    ...
    let created;
    try {
        created = await apiPost(cfg, `/${userPath}/threads`, createParams);
    }
    catch (err) {
    ...
    for (let attempt = 0; attempt < 8; attempt++) {
        try {
            const published = await apiPost(cfg, `/${userPath}/threads_publish`, {
            ...
            lastErr = err;
            if (attempt >= 7 || !isTransientPublishError(err))
                throw err;
    ...
    let permalink;
    try {
        const media = await apiGet(cfg, `/${mediaId}`, { fields: 'permalink' });
...
async function publish(cfg, text, replyToId, imageUrl) {
    ...
    try {
        ...
        if (primary !== 'me' && isMissingResourceError(err)) {
            console.warn(`[threads] publish via user id ${primary} failed (${errText(err)}); retrying as me`);
            return publishAgainstUser(cfg, 'me', text, replyToId, imageUrl);
        }
        throw err;
    }
}
async function publishPost(cfg, text, imageUrl) {
    return publish(cfg, text, undefined, imageUrl);
}
async function publishReply(cfg, text, replyToId) {
    const target = replyToId.trim();
    if (!target)
        throw new Error('Threads API: reply target id is empty');
    // Avoid create+publish round-trips against deleted/expired media ids.
    if (!(await threadsMediaExists(cfg, target))) {
        throw new Error(`Threads API: reply target ${target} no longer exists (deleted or expired). Skipping this reply.`);
    }
    try {
...
async function scrapeRecentTexts(cfg, count) {
 ...
 */
async function searchKeywordPosts(cfg, keyword, limit = 15) {
    const q = keyword.trim();
    if (!q)
        return { ok: false, posts: [], message: 'Keyword is empty' };
    const n = Math.max(1, Math.min(50, Math.floor(limit) || 15));
    try {
        // Endpoint is app-root level: /keyword_search (not under /me).
        const data = await apiGet(cfg, '/keyword_search', {
            q,
            search_type: 'RECENT',
            search_mode: 'KEYWORD',
            media_type: 'TEXT',
        ...
        for (const p of data.data ?? []) {
            if (typeof p.id !== 'string' || !p.id)
                continue;
            if (p.is_reply === true)
                continue; // prefer root posts for discovery replies
...
async function fetchAllReplyPages(cfg, path, maxPages = 5) {
    const out = [];
    let after;
    for (let page = 0; page < maxPages; page++) {
        const params = { fields: REPLY_FIELDS, limit: '100' };
        if (after)
            params.after = after;
        const data = await apiGet(cfg, path, params);
 ...
 * top-level comment.
 */
async function fetchReplyMessages(cfg, postId, expandBudget) {
    try {
        ...
        const all = [...top];
        const seen = new Set(top.map((r) => r.id).filter((id) => typeof id === 'string'));
        const queue = [...top];
        while (queue.length > 0 && expandBudget.left > 0) {
            ...
            if (parent.has_replies === false)
                continue;
            expandBudget.left--;
            try {
                const children = await fetchAllReplyPages(cfg, `/${parent.id}/replies`, 3);
                for (const child of children) {
                    if (typeof child.id !== 'string' || !child.id || seen.has(child.id))
                        continue;
                    seen.add(child.id);
                    all.push(child);
                    queue.push(child);
...
async function isAnsweredByMe(cfg, replyId, myUsername) {
    const me = myUsername.toLowerCase();
    if (!me)
        return false;
    try {
        const data = await apiGet(cfg, `/${replyId}/replies`, {
            fields: 'id,username',
            limit: '50',
        });
 ...
 */
async function fetchUnansweredPostReplies(cfg) {
    const me = await apiGet(cfg, '/me', { fields: 'id,username' });
    ...
    for (const post of posts) {
        ...
        for (const m of items) {
            ...
            if (m.id === post.id)
                continue;
            const uname = (m.username ?? '').toLowerCase();
            if (!uname || (myUsername && uname === myUsername))
                continue;
 ...
 */
async function fetchUnansweredMentions(cfg) {
    const me = await apiGet(cfg, '/me', { fields: 'id,username' });
    const myUsername = (typeof me.username === 'string' ? me.username : '').toLowerCase();
    // Recent window — Meta requires since >= 1688540400.
    const since = Math.max(1688540400, Math.floor(Date.now() / 1000) - 14 * 24 * 3600);
    const fields = 'id,text,username,timestamp,permalink,shortcode,media_type,is_reply,owner{id,username}';
    let data;
    try {
    ...
    for (const m of raw) {
        ...
        let username = typeof m.username === 'string' && m.username.trim()
            ? m.username.trim()
 ...
 */
async function fetchUnansweredReplies(cfg, opts) {
    const result = await fetchUnansweredEngagement(cfg, opts);
    return result.replies;
}
async function fetchUnansweredEngagement(cfg, opts) {
