...
exports.startThreadsOAuth = startThreadsOAuth;
...
const normalizeScopes = (raw) => raw
    .split(/[,\s]+/)
    .map((s) => s.trim())
    .filter(Boolean)
    .join(',');
function writeCallbackPage(res, title, body) {
...
async function readJson(label, res) {
...
async function exchangeCode(cfg, code) {
    ...
    if (!shortToken.access_token)
        throw new Error('Threads OAuth short-token response did not include access_token');
    ...
    if (!longToken.access_token)
        throw new Error('Threads OAuth long-token response did not include access_token');
...
async function startThreadsOAuth(cfg) {
    try {
        if (!cfg.appId.trim())
            return { ok: false, message: 'Threads app ID is required' };
        if (!cfg.appSecret.trim())
            return { ok: false, message: 'Threads app secret is required' };
        const redirectUri = new URL(cfg.redirectUri.trim());
        const scopes = normalizeScopes(cfg.scopes);
        if (!scopes)
            return { ok: false, message: 'At least one Threads scope is required' };
        ...
        const test = await (0, threadsApi_1.testThreads)({ accessToken: token.accessToken, userId: token.userId });
        if (!test.ok)
